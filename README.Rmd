...
This package performs regression analysis to investigate the impact of covariates on the time-dependent AUC under competing risks, based on the manuscript "Analyzing Heterogeneity in Biomarker Discriminative Performance under Competing Risks".
...
The function `covariance_cal` computes three matrices: \(\Sigma_1\), \(\Sigma_2\), and \(V\). The asymptotic variance, \(V\), is defined as:
...
You can also estimate the time-dependent AUC with confidence interval for each cause of failure. The following code estimates the time-dependent AUC for cause 1, given xc = 1 and xd = 0 at time t0 ranges from 0 to 10:
